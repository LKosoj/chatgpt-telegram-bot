...
Проект сохраняет UX чата / изображений / аудио / vision из оригинальной линейки
`chatgpt-telegram-bot`, но сейчас сосредоточен на gateway-моделях, развитой
системе плагинов, именованных SQLite-сессиях, опциональной долговременной
памяти Hindsight, рантайме субагентов / скилов и поддержке Model Context
Protocol (MCP).

> **Статус.** Кодовая база значительно разошлась с upstream-проектом.
> Дефолтные модели, маршрутизация изображений/аудио, неймспейсинг плагинов,
> реестр чат-режимов, рантайм агентов/скилов и расположение хранилища —
...
- **Telegram-ассистент с tools**: OpenAI-совместимый chat runtime с
  неймспейсингом инструментов плагинов, JSON-Schema-валидацией аргументов,
  allow-list’ами инструментов на уровне чат-режимов и прямой доставкой
  артефактов обратно в Telegram.
- **Агенты и сабагенты**: `agent_tools` умеет запускать ограниченных сабагентов,
  задавать пользователю уточняющие вопросы в Telegram, вести план и доставлять
  сгенерированные файлы, изображения, аудио или текстовые артефакты.
...
| `bot/hindsight_client.py` | HTTP-клиент Hindsight (recall, retain, list, stats) |
...
| `PLUGIN_STORAGE_ROOT` | `<repo>/data` | path | Корень состояния плагинов (skills workdir, agent tasks, MCP-конфиг, conversation analytics, …). Создаётся при старте. |
...
Плагины могут регистрировать свои слэш-команды и обработчики кнопок через
`Plugin.get_commands()` и `Plugin.get_message_handlers()`. Они находятся
через `PluginManager.build_bot_commands()` и регистрируются рядом со
встроенными. Команды с `add_to_menu=True` также показываются в меню
`/plugins`.
...
`_register_message_tail_handlers()` регистрирует следующие фильтры:
...
| `text_document_qa` | `upload_document`, `ask_question`, `list_documents`, `delete_document` | Один workspace AnythingLLM на чат. |
...
| Плагин | Инструменты | Заметки |
|---|---|---|
| `agent_tools` | `manage_plan_tasks`, `ask_telegram_user`, `cancel_pending_question`, `deliver_to_user`, `run_subagents` | См. [Рантайм агентов](#рантайм-агентов--субагентов). |
...
| `hindsight_memory` | `recall`, `list_memories`, `stats` | Ручная инспекция Hindsight. |
...
- **`run_subagents`** — параллельно запускает до 5 ограниченных субагентов.
  ...
  `deliver_to_user`, `cancel_pending_question`.
...
- **`manage_plan_tasks`** — единый инструмент трекинга плана для текущего
  scope с `action ∈ {add, update, list, clear}` и списком задач (у каждой
  свой `id` вида `T1`, свободный `content` и `status`). Статусы:
  `pending`, `in_progress`, `completed`, `cancelled`. Задачи старше
...
как обычный инструмент, когда нужна ручная инспекция памяти.
...
│   ├── agent_tasks.json        # Persistent-план
