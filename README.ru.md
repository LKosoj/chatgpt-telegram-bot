...
| `SUBAGENT_BLOCKED_FUNCTIONS` | 5 self-protection-инструментов | константа | Какие инструменты субагент вызвать не может (`agent_tools.run_subagents`, `…ask_telegram_user`, `…deliver_to_user`, `…cancel_pending_question`, `skills.run_skill_agent`). |
...
скил с файлом `SKILL.md`, описывающим намерение, опциональными скриптами и
опциональными specialist-профилями `agents/*.yaml`) как инструменты, которые
модель может вызывать.
...
- `run_skill_agent` — лениво создать сабагента из профиля `agents/*.yaml`
  внутри скила и запустить его через `agent_tools.run_subagents` только тогда,
  когда модель явно решила использовать этого специалиста.
